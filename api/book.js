...
const NOTIFY = process.env.BOOKING_NOTIFY_EMAIL || 'dev@vedryxtech.com'
const IMPERSONATE = process.env.GOOGLE_CALENDAR_IMPERSONATE || 'dev@vedryxtech.com'
const FROM = process.env.BOOKING_FROM || 'vedryxTech <booking@team.vedryxtech.com>'
...
// IST is a fixed +05:30 offset (no DST) — safe to build an absolute instant.
...
function prettyWhen(dateStr, timeStr) {
  const end = SLOTS.includes(timeStr)
    ? new Date(istInstant(dateStr, timeStr).getTime() + SLOT_MINUTES * 60000)
    : null
  const dateFmt = new Intl.DateTimeFormat('en-GB', {
    timeZone: TZ, weekday: 'long', day: 'numeric', month: 'long', year: 'numeric',
  }).format(istInstant(dateStr, timeStr))
  const t = (d) => new Intl.DateTimeFormat('en-GB', {
    timeZone: TZ, hour: '2-digit', minute: '2-digit', hour12: true,
  }).format(d)
  const startT = t(istInstant(dateStr, timeStr))
  const endT = end ? t(end) : ''
  return `${dateFmt} · ${startT}–${endT} IST`
}
...
function guestEmailHtml({ name, whenStr, meetLink }) {
  ...
  return `<!doctype html><html><body style="margin:0;background:#0B0F1A;font-family:Georgia,'Times New Roman',serif;color:#F4F6FB;">
  <div style="max-width:520px;margin:0 auto;padding:36px 24px;">
    <div style="font-size:12px;letter-spacing:.32em;text-transform:uppercase;color:#8FB2FF;">vedryxTech</div>
    <h1 style="font-size:24px;font-weight:500;margin:14px 0 8px;">Your demo is booked</h1>
    <p style="margin:0 0 18px;line-height:1.6;color:rgba(244,246,251,.8);">${hi} thanks for booking a 15-minute demo with vedryxTech. Details below.</p>
    <div style="border:1px solid rgba(31,95,255,.35);border-radius:6px;padding:18px 20px;margin:0 0 20px;background:rgba(31,95,255,.06);">
      <div style="font-size:12px;letter-spacing:.14em;text-transform:uppercase;color:rgba(244,246,251,.55);margin-bottom:6px;">When</div>
      <div style="font-size:17px;margin-bottom:16px;">${escapeHtml(whenStr)}</div>
      <div style="font-size:12px;letter-spacing:.14em;text-transform:uppercase;color:rgba(244,246,251,.55);margin-bottom:6px;">Google Meet</div>
      <a href="${escapeAttr(meetLink)}" style="color:#8FB2FF;font-size:15px;word-break:break-all;">${escapeHtml(meetLink)}</a>
    </div>
    <a href="${escapeAttr(meetLink)}" style="display:inline-block;background:#1F5FFF;color:#fff;text-decoration:none;padding:12px 22px;border-radius:4px;font-size:15px;">Join the Meet</a>
    <p style="margin:22px 0 0;font-size:13px;line-height:1.6;color:rgba(244,246,251,.5);">A Google Calendar invite with the same link is on its way. Need to reschedule? Just reply to this email.</p>
    <p style="margin:20px 0 0;font-size:11px;letter-spacing:.28em;text-transform:uppercase;color:rgba(244,246,251,.3);">vedryxtech.com</p>
  </div></body></html>`
}
...
export default async function handler(req, res) {
  ...
  if (date < today || date > maxDate) return res.status(400).json({ ok: false, error: 'That date is outside the booking window.' })
  ...
  // ---- create the calendar event with a real Meet link ----
  let meetLink = ''
  let eventId = ''
  try {
  ...
  // ---- Resend confirmations (guest + host). Non-fatal if email fails. ----
  const emailWarnings = []
  try {
    await sendEmail({
      from: FROM, to: [email], reply_to: REPLY_TO,
      subject: `Your vedryxTech demo — ${whenStr}`,
      html: guestEmailHtml({ name, whenStr, meetLink }),
    })
