...
export default function BookingModal({ open, onClose }) {
  ...
  return (
    <div className="modal-scrim" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal" role="dialog" aria-modal="true" aria-label="Book a 15-minute demo" ref={dialogRef}>
        ...
        {status === 'done' ? (
          <div className="book-done">
            <div className="book-tick" aria-hidden="true">
              <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#8FB2FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
            </div>
          ...
          <form className="book-form" onSubmit={submit}>
              ...
              <div className="chip-row" role="listbox" aria-label="Choose a date">
                {days.map((d) => (
                  <button
                    type="button"
                    key={d.value}
                    className={`chip chip-day${date === d.value ? ' chip--on' : ''}`}
            ...
            <div className="field">
              ...
              <div className="slot-grid">
                {slotsForDay.map(({ t, disabled }) => (
                  <button
                    ...
                    key={t}
                    disabled={disabled}
                    className={`chip chip-slot${time === t ? ' chip--on' : ''}`}
              ...
              {slotsForDay.every((s) => s.disabled) && (
                <p className="field-hint">No slots left today — pick another date.</p>
              )}
            </div>
